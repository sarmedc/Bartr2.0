...
import com.example.sarmedchaudhry.bartr20.R;
...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String users_name = logInData.getString("users_name");
        dbHelper =(DBHelper) getIntent().getSerializableExtra("db");
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        arguments.putSerializable("db",dbHelper);

        if (id == R.id.nav_my_list) {
            MyList myFragment = new MyList();
            myFragment.setArguments(arguments);
            fragmentManager.beginTransaction().replace(R.id.content_frame, myFragment).commit();
        } else if (id == R.id.nav_browse) {
            fragmentManager.beginTransaction().replace(R.id.content_frame, new Browse()).commit();
